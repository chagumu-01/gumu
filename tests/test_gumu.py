...
def build_agent(tmp_path, outputs, **kwargs):
    ...
    store = SessionStore(tmp_path / ".gumu" / "sessions")
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".gumu").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".gumu" not in result
...
def test_welcome_screen_keeps_box_shape_for_long_paths(tmp_path):
    deep = tmp_path / "very" / "long" / "path" / "for" / "the" / "gumu" / "agent" / "welcome" / "screen"
    ...
    visible_welcome = ansi_re.sub("", welcome)

    assert len(lines) >= 5
    # Check visible line lengths are consistent (ANSI codes don't count)
    visible_lengths = {len(ansi_re.sub("", line)) for line in lines}
    assert visible_lengths == {len(ansi_re.sub("", lines[0]))}
    assert "██" in visible_welcome
    assert "MINI-CODING-AGENT" not in visible_welcome
    assert "MINI CODING AGENT" not in visible_welcome
    assert "gumu" in visible_welcome
    assert "local coding agent" in visible_welcome
    assert "// READY" not in visible_welcome
    assert "SLASH" not in visible_welcome
    assert "READY      " not in visible_welcome
    assert "commands: Commands:" not in visible_welcome
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "gumu/0.1"
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "gumu.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("gumu.cli.OpenAICompatibleModelClient") as mock_openai:
            fake_client = mock_openai.return_value
            agent = gumu_pkg.build_agent(args)
...
def test_build_agent_uses_right_codes_shared_key_for_openai_provider(tmp_path):
    ...
    with patch.dict(os.environ, {"GUMU_RIGHT_CODES_API_KEY": "sk-right-codes"}, clear=True):
        with patch(
            "gumu.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("gumu.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_arg_parser_defaults_provider_to_deepseek(tmp_path):
    args = gumu_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path)])

    assert args.provider == "deepseek"


def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = gumu_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])

    assert args.provider == "anthropic"


def test_build_arg_parser_accepts_deepseek_provider(tmp_path):
    args = gumu_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "gumu.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("gumu.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("gumu.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            gumu_pkg.build_agent(args)
    ...
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                ...
                "GUMU_DEEPSEEK_API_KEY=sk-project-deepseek",
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .gumu/.\n"
    ...
    index_path = tmp_path / ".gumu" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".gumu" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".gumu" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .gumu/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".gumu" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".gumu" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(gumu_pkg.__file__).as_posix().endswith("/gumu/__init__.py")
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(gumu_pkg.main)
    assert callable(gumu_pkg.build_agent)
    assert callable(gumu_pkg.build_arg_parser)


def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "gumu", "--help"],
        capture_output=True,
        text=True,
    )
