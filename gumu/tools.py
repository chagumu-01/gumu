...
def tool_run_shell(context, args):
    ...
    if os.name == "nt":
        try:
            ...
            parts = []
        if len(parts) >= 3 and parts[0] == "printf" and parts[1] == "%s":
    ...
    if os.name == "nt":
        ...
        env.setdefault("ComSpec", os.environ.get("ComSpec") or os.environ.get("COMSPEC") or rf"{system_root}\System32\cmd.exe")
        env.setdefault("COMSPEC", env["ComSpec"])
        env.setdefault("PATHEXT", os.environ.get("PATHEXT") or ".COM;.EXE;.BAT;.CMD")
    shell_executable = env.get("ComSpec") if os.name == "nt" else None
    result = subprocess.run(
        command,
        cwd=context.root,
        shell=True,
